...
- Music / SFX → their own timeline lane: drag an audio asset from 素材 onto the timeline to place it at that moment (or upload/generate from the toolbar 音频 panel). Drag a chip to reposition, click to select (Del removes it), and set that track's volume / fade-in / fade-out / speed in the 音频 panel. Several tracks may overlap — their sounds sum. Or ask the agent: set_bgm.
