...
- **Workbench** (`/studio/<id>`), one screen, four zones:
  ...
  - **Timeline** (bottom) — track 0 is the talking-head video (scene cards with framing badges and the footage's own waveform under each thumbnail strip), upper tracks are overlay components; captions do not occupy a track (edit them via 字幕/剪口播). Drag assets in, drag clips around, box-select.
