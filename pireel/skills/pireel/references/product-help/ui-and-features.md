...
- Music / SFX → their own timeline lane (bottom), drawn with waveforms: drag an audio asset from 素材 onto the timeline to place it there, or upload/generate from the toolbar 音频 panel (those land at the playhead). On a clip: body-drag moves it, the two edges trim in/out, the top-corner knobs drag the fades, click selects and Del removes. The 音频 panel has the track list plus the selected clip's volume / fade-in / fade-out / speed sliders. Several tracks may overlap — their sounds sum. Or ask the agent: set_bgm.
