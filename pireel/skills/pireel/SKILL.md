...
- **Two element kinds.** Overlay **blocks** = designed graphics laid over the video (metric cards, comparisons, charts, callouts, titles) — the main event. Video **shots** = segments of the talking-head clip, each with a framing treatment (`full` / `punch-in` / `corner-br`|`corner-tl` / `split-l`|`split-r`|`split-t`|`split-b`; the split axis follows the canvas — portrait splits top/bottom, landscape splits left/right). Cuts are hard jumps; visual variety comes from framing, not transitions.
...
`2026-07-29.1`. On connect, the Pireel MCP server announces its current skill baseline in its instructions. The format is `YYYY-MM-DD.rev` — compare the date first, then the numeric `.rev` (e.g. `2026-07-21.3` is newer than `2026-07-21.2`; a bare `2026-07-21` counts as `.0`). If the server's baseline is **newer** than the version above, update the skill YOURSELF before continuing: run `npx skills update pireel` (fallback: `npx skills add pireel/pireel-agent`) — the host's command-approval flow is the user's consent, don't bounce it to them — then re-read the updated files. Only if the command is unavailable or denied, mention it once and continue. Don't nag if versions match.
